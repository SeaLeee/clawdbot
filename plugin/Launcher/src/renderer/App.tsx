...
interface OneClickProgressData {
  service: "openclaw" | "crabwalk" | "all";
  step: "starting" | "waiting" | "opening" | "done" | "error";
  message: string;
}
...
declare global {
  interface Window {
    api: {
      ...
      oneClickOpenAll: () => Promise<{
        openclaw: { success: boolean; url?: string; error?: string };
        crabwalk: { success: boolean; url?: string; error?: string };
      }>;
      oneClickOpenSkillDashboard: () => Promise<{
        success: boolean;
        url?: string;
        alreadyRunning?: boolean;
        error?: string;
      ...
      onServiceStarted: (
        callback: (_event: unknown, data: ServiceStartedData) => void,
      ) => () => void;
      onOneClickProgress: (
        callback: (_event: unknown, data: OneClickProgressData) => void,
      ) => () => void;
    };
  }
}
...
function App() {
  ...
  // 一键打开状态
  const [oneClickLoading, setOneClickLoading] = useState<{
    openclaw: boolean;
    crabwalk: boolean;
    all: boolean;
    dashboard: boolean;
  }>({ openclaw: false, crabwalk: false, all: false, dashboard: false });
  const [oneClickProgress, setOneClickProgress] = useState<{
    openclaw?: string;
    crabwalk?: string;
    all?: string;
    "skill-dashboard"?: string;
  }>({});
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.api.onServiceStarted((_event, data) => {
      ...
      if (data.success) {
        ...
        setAutoStartLogs((prev) => [
          ...prev,
          `[${timestamp}] ✗ ${data.name} 自动启动失败: ${data.error}`,
        ]);
      }
    });
  ...
  // 监听一键打开进度事件
  useEffect(() => {
    if (!window.api?.onOneClickProgress) {
      return;
    }

    const unsubscribe = window.api.onOneClickProgress((_event, data) => {
      setOneClickProgress((prev) => ({ ...prev, [data.service]: data.message }));
      addLog(`⚡ [${data.service}] ${data.message}`);

      if (data.step === "done" || data.step === "error") {
        // 3 秒后清除进度信息
        setTimeout(() => {
          setOneClickProgress((prev) => ({ ...prev, [data.service]: undefined }));
        }, 3000);
      }
    });
  ...
  // 一键打开 OpenClaw
  const handleOneClickOpenClaw = async () => {
    setOneClickLoading((prev) => ({ ...prev, openclaw: true }));
    setOneClickProgress((prev) => ({ ...prev, openclaw: "准备中..." }));
    addLog("⚡ 一键打开 OpenClaw...");

    try {
      const result = await window.api?.oneClickOpenOpenClaw?.();
      if (result?.success) {
        addLog(
          `✓ 一键打开 OpenClaw 成功: ${result.url}${result.alreadyRunning ? " (已在运行)" : ""}`,
        );
        setMessage(result.alreadyRunning ? "已打开 OpenClaw 面板" : "OpenClaw 已启动并打开面板");
        setTimeout(() => {
          window.api
            ?.getProcessStatus?.()
            .then(setProcessStatus)
            .catch(() => {});
        }, 1000);
      } else {
        addLog(`✗ 一键打开 OpenClaw 失败: ${result?.error}`);
        setMessage(`失败: ${result?.error}`);
      }
    } catch (error) {
      addLog(`✗ 一键打开 OpenClaw 异常: ${error}`);
      setMessage(`错误: ${error}`);
    } finally {
      setOneClickLoading((prev) => ({ ...prev, openclaw: false }));
    }
  };

  // 一键打开 Crabwalk
  const handleOneClickCrabwalk = async () => {
    setOneClickLoading((prev) => ({ ...prev, crabwalk: true }));
    setOneClickProgress((prev) => ({ ...prev, crabwalk: "准备中..." }));
    addLog("⚡ 一键打开 Crabwalk...");

    try {
      const result = await window.api?.oneClickOpenCrabwalk?.();
      if (result?.success) {
        addLog(
          `✓ 一键打开 Crabwalk 成功: ${result.url}${result.alreadyRunning ? " (已在运行)" : ""}`,
        );
        setMessage(result.alreadyRunning ? "已打开 Crabwalk 面板" : "Crabwalk 已启动并打开面板");
        setTimeout(() => {
          window.api
            ?.getProcessStatus?.()
            .then(setProcessStatus)
        ...
        addLog(`✗ 一键打开 Crabwalk 失败: ${result?.error}`);
        setMessage(`失败: ${result?.error}`);
      }
    } catch (error) {
      addLog(`✗ 一键打开 Crabwalk 异常: ${error}`);
      setMessage(`错误: ${error}`);
    } finally {
      setOneClickLoading((prev) => ({ ...prev, crabwalk: false }));
    }
  };

  // 一键打开 Skill Dashboard
  const handleOneClickDashboard = async () => {
    setOneClickLoading((prev) => ({ ...prev, dashboard: true }));
    setOneClickProgress((prev) => ({ ...prev, "skill-dashboard": "准备中..." }));
    addLog("⚡ 一键打开 Skill Dashboard...");

    try {
      const result = await window.api?.oneClickOpenSkillDashboard?.();
      if (result?.success) {
        addLog(`✓ 一键打开 Skill Dashboard 成功: ${result.url}`);
        setMessage("已打开 Skill Dashboard");
        setTimeout(() => {
          window.api
            ?.getProcessStatus?.()
            .then(setProcessStatus)
        ...
        addLog(`✗ 一键打开 Skill Dashboard 失败: ${result?.error}`);
        setMessage(`失败: ${result?.error}`);
      }
    } catch (error) {
      addLog(`✗ 一键打开 Skill Dashboard 异常: ${error}`);
      setMessage(`错误: ${error}`);
    } finally {
      setOneClickLoading((prev) => ({ ...prev, dashboard: false }));
    }
  };

  // 一键全部打开
  const handleOneClickAll = async () => {
    setOneClickLoading((prev) => ({ ...prev, all: true }));
    setOneClickProgress((prev) => ({ ...prev, all: "正在启动所有服务..." }));
    addLog("⚡ 一键打开所有服务...");

    try {
      const results = await window.api?.oneClickOpenAll?.();
      const msgs: string[] = [];
      if (results?.openclaw?.success) {
        msgs.push("OpenClaw ✓");
      } else {
        msgs.push(`OpenClaw ✗ ${results?.openclaw?.error || ""}`);
      }
      if (results?.crabwalk?.success) {
        msgs.push("Crabwalk ✓");
      } else {
        msgs.push(`Crabwalk ✗ ${results?.crabwalk?.error || ""}`);
      }

      addLog(`✓ 一键全部打开完成: ${msgs.join(", ")}`);
      setMessage(`全部打开: ${msgs.join(", ")}`);
      setTimeout(() => {
        window.api
          ?.getProcessStatus?.()
          .then(setProcessStatus)
      ...
      addLog(`✗ 一键全部打开异常: ${error}`);
      setMessage(`错误: ${error}`);
    } finally {
      setOneClickLoading((prev) => ({ ...prev, all: false }));
    }
  };
  ...
  const SourceBadge = ({
    ...
    if (source === "project") {
      return (
        ...
        >
          📁 项目默认{defaultSource ? ` (${defaultSource})` : ""}
        </span>
      );
    }
    return (
          ...
          <div className="tab-content">
            ...
            <div className="one-click-section">
              <div className="one-click-header">
                <Zap className="one-click-icon" />
                <h2>一键打开</h2>
                <span className="one-click-subtitle">启动服务并自动打开面板，无需手动操作</span>
              </div>
              <div className="one-click-buttons">
                <button
                  className={`one-click-btn one-click-openclaw ${oneClickLoading.openclaw ? "loading" : ""}`}
                  onClick={handleOneClickOpenClaw}
                  disabled={oneClickLoading.openclaw || oneClickLoading.all}
                >
                  <div className="one-click-btn-content">
                    <Zap size={22} />
                    <div className="one-click-btn-text">
                      <span className="one-click-btn-title">OpenClaw</span>
                      <span className="one-click-btn-desc">
                        {oneClickProgress.openclaw ||
                          (processStatus.openclaw ? "运行中 · 点击打开面板" : "启动并打开面板")}
                      </span>
                    </div>
                  </div>
                  {oneClickLoading.openclaw && <div className="one-click-spinner" />}
                  {!oneClickLoading.openclaw && processStatus.openclaw && (
                    <div className="one-click-status-dot active" />
                  )}
                </button>

                <button
                  className={`one-click-btn one-click-crabwalk ${oneClickLoading.crabwalk ? "loading" : ""}`}
                  onClick={handleOneClickCrabwalk}
                  disabled={oneClickLoading.crabwalk || oneClickLoading.all}
                >
                  <div className="one-click-btn-content">
                    <Zap size={22} />
                    <div className="one-click-btn-text">
                      <span className="one-click-btn-title">Crabwalk</span>
                      <span className="one-click-btn-desc">
                        {oneClickProgress.crabwalk ||
                          (processStatus.crabwalk ? "运行中 · 点击打开面板" : "启动并打开面板")}
                      </span>
                    </div>
                  </div>
                  {oneClickLoading.crabwalk && <div className="one-click-spinner" />}
                  {!oneClickLoading.crabwalk && processStatus.crabwalk && (
                    <div className="one-click-status-dot active" />
                  )}
                </button>

                <button
                  className={`one-click-btn one-click-dashboard ${oneClickLoading.dashboard ? "loading" : ""}`}
                  onClick={handleOneClickDashboard}
                  disabled={oneClickLoading.dashboard || oneClickLoading.all}
                >
                  <div className="one-click-btn-content">
                    <Zap size={22} />
                    <div className="one-click-btn-text">
                      <span className="one-click-btn-title">Skill Dashboard</span>
                      <span className="one-click-btn-desc">
                        {oneClickProgress["skill-dashboard"] ||
                          (processStatus.openclaw ? "运行中 · 打开技能面板" : "启动并打开技能面板")}
                      </span>
                    </div>
                  </div>
                  {oneClickLoading.dashboard && <div className="one-click-spinner" />}
                  {!oneClickLoading.dashboard && processStatus.openclaw && (
                    <div className="one-click-status-dot active" />
                  )}
                </button>

                <button
                  className={`one-click-btn one-click-all ${oneClickLoading.all ? "loading" : ""}`}
                  onClick={handleOneClickAll}
                  disabled={
                    oneClickLoading.openclaw || oneClickLoading.crabwalk || oneClickLoading.all
                  }
                >
                  <div className="one-click-btn-content">
                    <Rocket size={22} />
                    <div className="one-click-btn-text">
                      <span className="one-click-btn-title">全部打开</span>
                      <span className="one-click-btn-desc">
                        {oneClickProgress.all || "同时启动 OpenClaw + Crabwalk"}
                      </span>
                    </div>
                  </div>
                  {oneClickLoading.all && <div className="one-click-spinner" />}
                </button>
              </div>
            </div>
            ...
            <div className="launcher-grid">
              ...
              <div className="service-card">
                <div className="card-header">
                  <h2>OpenClaw</h2>
                  <div className={`status-indicator ${processStatus.openclaw ? "active" : ""}`}>
                ...
                {detectedPanels.openclaw && (
                  ...
                  >
                    <div>
                      📍 地址: {detectedPanels.openclaw.ip}:{detectedPanels.openclaw.port}
                    </div>
                    <div>🔍 来源: {detectedPanels.openclaw.source}</div>
                    {detectedPanels.openclaw.message && (
                      <div>💬 {detectedPanels.openclaw.message}</div>
                    )}
                  </div>
                )}
              ...
              <div className="service-card">
                <div className="card-header">
                  <h2>Crabwalk</h2>
                  <div className={`status-indicator ${processStatus.crabwalk ? "active" : ""}`}>
                    <div className="status-dot"></div>
                ...
                {detectedPanels.crabwalk && (
                  ...
                  >
                    <div>
                      📍 地址: {detectedPanels.crabwalk.ip}:{detectedPanels.crabwalk.port}
                    </div>
                    <div>🔍 来源: {detectedPanels.crabwalk.source}</div>
                    {detectedPanels.crabwalk.message && (
                      <div>💬 {detectedPanels.crabwalk.message}</div>
                    )}
                  </div>
                )}
              ...
              <div className="setting-group">
                ...
                >
                  ...
                  <p style={{ color: "#666", fontSize: "0.9rem" }}>
                    前往「自动启动」标签页，可以配置开机后自动启动 OpenClaw、Crabwalk 并自动打开
                    WebUI。
                  </p>
                  <button
                    className="btn btn-secondary"
            ...
            <div className="settings-container">
              <div className="setting-group">
                <div className="setting-header">
                  ...
                  <p className="setting-description">
                    配置开机后自动启动的服务和行为。配置保存在{" "}
                    <code>{configPath || "~/.clawdbot-launcher/autostart.json"}</code>
                  </p>
                </div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setAutoStartConfig((prev) => ({
                        ...prev,
                        launchAgentEnabled: !prev.launchAgentEnabled,
                      }));
                    }}
                    disabled={loading || !window.api?.enableLaunchAgent}
                    title={
                      !window.api?.enableLaunchAgent
                        ? "开机启动需要 Electron 应用"
                        : autoStartConfig.launchAgentEnabled
                          ? "点击禁用"
                          : "点击启用"
                    }
                  >
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      handleSaveAutoStartConfig({
                        autoStartOpenClaw: !autoStartConfig.autoStartOpenClaw,
                      })
                    }
                    disabled={loading}
                ...
                >
                  <div className="setting-info">
                    <h4 style={{ fontSize: "0.95rem", color: "#555" }}>
                      📱 自动打开 OpenClaw WebUI
                    </h4>
                  ...
                  <button
                    ...
                    onClick={() =>
                      handleSaveAutoStartConfig({
                        autoOpenOpenClawPanel: !autoStartConfig.autoOpenOpenClawPanel,
                      })
                    }
                    disabled={loading || !autoStartConfig.autoStartOpenClaw}
                    title={
                      !autoStartConfig.autoStartOpenClaw
                        ? "需先启用自动启动 OpenClaw"
                        : autoStartConfig.autoOpenOpenClawPanel
                          ? "点击禁用"
                          : "点击启用"
                    }
                  >
                  ...
                  <div className="setting-info" style={{ flex: 1 }}>
                    ...
                    >
                      ...
                      <SourceBadge
                        source={getFieldSource(
                          ...
                          projectDefaults.gatewayToken,
                        )}
                        defaultSource={projectDefaults.sources.gatewayToken}
                      />
                    </h4>
                    <p style={{ fontSize: "0.85rem" }}>
                      用于修复 disconnected (1008): unauthorized。请从 Dashboard URL 复制 token
                      后粘贴。
                    </p>
                    ...
                    <div style={{ position: "relative", marginTop: "8px" }}>
                      <input
                        ...
                        placeholder={
                          projectDefaults.gatewayToken
                            ? `留空使用项目默认 (${maskValue(projectDefaults.gatewayToken)})`
                            : "粘贴 gateway token"
                        }
                        ...
                        onChange={(e) =>
                          setAutoStartConfig((prev) => ({
                            ...prev,
                            openclawGatewayToken: e.target.value,
                          }))
                        }
                        onBlur={(e) =>
                          handleSaveAutoStartConfig({ openclawGatewayToken: e.target.value || "" })
                        }
                        disabled={loading}
                        style={{
                          width: "100%",
                ...
                >
                  <div className="setting-info" style={{ flex: 1 }}>
                    ...
                    <p style={{ fontSize: "0.85rem" }}>
                      选择启动 OpenClaw 时使用的模型类型：MiniMax 或本地模型。
                    </p>
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setAutoStartConfig((prev) => ({
                          ...prev,
                          openclawModelProvider: provider,
                        }));
                      ...
                      <div style={{ marginTop: "10px", display: "grid", gap: "8px" }}>
                        <div>
                          ...
                          <div style={{ position: "relative" }}>
                            <input
                              ...
                              placeholder={
                                projectDefaults.minimaxApiKey
                                  ? `留空使用项目默认 (${maskValue(projectDefaults.minimaxApiKey)})`
                                  : "MiniMax API Key"
                              }
                              ...
                              onChange={(e) =>
                                setAutoStartConfig((prev) => ({
                                  ...prev,
                                  openclawMinimaxApiKey: e.target.value,
                                }))
                              }
                              onBlur={(e) =>
                                handleSaveAutoStartConfig({
                              ...
                              style={{
                                width: "100%",
                                padding: "8px 10px",
                                paddingRight: autoStartConfig.openclawMinimaxApiKey
                                  ? "60px"
                                  : "10px",
                                borderRadius: "6px",
                                border: "1px solid #d9d9d9",
                        ...
                        <div>
                          ...
                          >
                            ...
                            <SourceBadge
                              source={getFieldSource(
                                autoStartConfig.openclawMinimaxModel,
                                projectDefaults.minimaxModel,
                              )}
                          ...
                          <input
                            ...
                            placeholder={
                              projectDefaults.minimaxModel
                                ? `留空使用项目默认 (${projectDefaults.minimaxModel})`
                                : "MiniMax 模型名（默认 MiniMax-M1）"
                            }
                            value={autoStartConfig.openclawMinimaxModel || ""}
                            onChange={(e) =>
                              setAutoStartConfig((prev) => ({
                                ...prev,
                                openclawMinimaxModel: e.target.value,
                              }))
                            }
                            onBlur={(e) =>
                              handleSaveAutoStartConfig({
                                openclawMinimaxModel: e.target.value || "MiniMax-M1",
                              })
                            }
                            disabled={loading}
                            style={{
                      ...
                      <div style={{ marginTop: "10px", display: "grid", gap: "8px" }}>
                        <div>
                          ...
                          >
                            ...
                            <SourceBadge
                              source={getFieldSource(
                                autoStartConfig.openclawLocalBaseUrl,
                                projectDefaults.localBaseUrl,
                              )}
                          ...
                          <input
                            ...
                            placeholder={
                              projectDefaults.localBaseUrl
                                ? `留空使用项目默认 (${projectDefaults.localBaseUrl})`
                                : "本地模型服务地址（如 http://127.0.0.1:11434/v1）"
                            }
                            value={autoStartConfig.openclawLocalBaseUrl || ""}
                            onChange={(e) =>
                              setAutoStartConfig((prev) => ({
                                ...prev,
                                openclawLocalBaseUrl: e.target.value,
                              }))
                            }
                            onBlur={(e) =>
                              handleSaveAutoStartConfig({
                                openclawLocalBaseUrl: e.target.value || "http://127.0.0.1:11434/v1",
                              })
                            }
                            disabled={loading}
                            style={{
                        ...
                        <div>
                          ...
                          >
                            ...
                            <SourceBadge
                              source={getFieldSource(
                                autoStartConfig.openclawLocalModelName,
                                projectDefaults.localModelName,
                              )}
                          ...
                          <input
                            ...
                            placeholder={
                              projectDefaults.localModelName
                                ? `留空使用项目默认 (${projectDefaults.localModelName})`
                                : "本地模型名（如 qwen2.5:7b）"
                            }
                            ...
                            onChange={(e) =>
                              setAutoStartConfig((prev) => ({
                                ...prev,
                                openclawLocalModelName: e.target.value,
                              }))
                            }
                            onBlur={(e) =>
                              handleSaveAutoStartConfig({
                        ...
                        <div>
                          ...
                          {!autoStartConfig.openclawLocalApiKey && projectDefaults.localApiKey && (
                            <p
                              style={{ fontSize: "0.8rem", color: "#1a56db", margin: "0 0 4px 0" }}
                            >
                          ...
                          <div style={{ position: "relative" }}>
                            <input
                              ...
                              placeholder={
                                projectDefaults.localApiKey
                                  ? `留空使用项目默认 (${maskValue(projectDefaults.localApiKey)})`
                                  : "本地模型 API Key（可选）"
                              }
                              ...
                              onChange={(e) =>
                                setAutoStartConfig((prev) => ({
                                  ...prev,
                                  openclawLocalApiKey: e.target.value,
                                }))
                              }
                              onBlur={(e) =>
                                handleSaveAutoStartConfig({
                              ...
                              style={{
                                width: "100%",
                                padding: "8px 10px",
                                paddingRight: autoStartConfig.openclawLocalApiKey ? "60px" : "10px",
                                borderRadius: "6px",
                                border: "1px solid #d9d9d9",
                                fontSize: "0.9rem",
                              }}
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      handleSaveAutoStartConfig({
                        autoStartCrabwalk: !autoStartConfig.autoStartCrabwalk,
                      })
                    }
                    disabled={loading}
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      handleSaveAutoStartConfig({
                        autoOpenCrabwalkPanel: !autoStartConfig.autoOpenCrabwalkPanel,
                      })
                    }
                    disabled={loading || !autoStartConfig.autoStartCrabwalk}
                    title={
                      !autoStartConfig.autoStartCrabwalk
                        ? "需先启用自动启动 Crabwalk"
                        : autoStartConfig.autoOpenCrabwalkPanel
                          ? "点击禁用"
                          : "点击启用"
                    }
                  >
                ...
                >
                  ...
                  <p style={{ fontSize: "0.8rem", color: "#666", marginBottom: "10px" }}>
                    💡 规则：如果 UI 中填写了值则使用手动配置，否则自动使用项目默认配置（来自
                    openclaw.json / .env）
                  </p>
                  <div style={{ fontSize: "0.9rem", color: "#444", lineHeight: "1.8" }}>
                    <div>
                      • 开机启动 Launcher: {autoStartConfig.launchAgentEnabled ? "✅" : "❌"}
                    </div>
                    <div>
                      • 自动启动 OpenClaw: {autoStartConfig.autoStartOpenClaw ? "✅" : "❌"}{" "}
                      {autoStartConfig.autoStartOpenClaw &&
                        autoStartConfig.autoOpenOpenClawPanel &&
                        "(自动打开面板)"}
                    </div>
                    ...
                    <div>
                      • 模型类型:{" "}
                      {autoStartConfig.openclawModelProvider === "local" ? "本地模型" : "MiniMax"}
                    </div>
                    {autoStartConfig.openclawModelProvider === "minimax" && (
                      <>
                        <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap" }}>
                    ...
                    <div>
                      • 自动启动 Crabwalk: {autoStartConfig.autoStartCrabwalk ? "✅" : "❌"}{" "}
                      {autoStartConfig.autoStartCrabwalk &&
                        autoStartConfig.autoOpenCrabwalkPanel &&
                        "(自动打开面板)"}
                    </div>
